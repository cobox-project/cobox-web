...
export default function AccountConversationsPage() {
  ...
  const [conversations, setConversations] = useState(
    allConversations.filter((c) => c.accountId === accountId)
  );
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [conversations, statusFilter, onlyMine, searchQuery]);
  ...
  const ChannelIcon = channelIcons[account.channel];
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <header className="flex h-12 shrink-0 items-center gap-3 border-b px-4">
        ...
        <div
          className={cn(
            ...
            channelStyles[account.channel].bg
          )}
        >
          <ChannelIcon
            className={cn(
              "h-3.5 w-3.5",
              channelStyles[account.channel].text
            )}
          />
        </div>
      ...
      {/* Two-panel: conversation list + detail */}
