...
```typescript
// packages/api/src/domain/conversation.ts
export const canTransition = (from: Status, action: StatusAction): Status | null => {
  const transitions: Record<Status, Partial<Record<StatusAction, Status>>> = {
    open: { complete: "completed", noAction: "no_action" },
    completed: { reopen: "open" },
    no_action: { reopen: "open" },
  }
...
### 3.2 Bi-temporal センチネル値

`valid_to` / `superseded_at` には NULL ではなく `TEMPORAL_INFINITY` を使用する。
B-tree インデックスが NULL を含む行で効率が落ちることを防ぐ。

```typescript
// packages/shared/src/types/temporal.ts

/** Bi-temporal カラムのセンチネル値（「現在有効」「未訂正」を表す） */
...
describe("canTransition", () => {
  it("open -> complete で completed に遷移する", () => {
    expect(canTransition("open", "complete")).toBe("completed")
  })
...
- [ ] Bi-temporal カラム（`valid_to`, `superseded_at`）に NULL ではなく `TEMPORAL_INFINITY`（`9999-12-31 23:59:59Z`）を使用しているか
