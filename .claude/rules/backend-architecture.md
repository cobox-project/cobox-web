...
│   │       │       ├── instagram.ts
│   │       │       ├── facebook.ts
│   │       │       ├── gmail.ts           # Gmail Push Notification
│   │       │       ├── outlook.ts         # Microsoft Graph Notification
...
│   │       │       ├── EmailGateway.ts       # Email 統合（プロバイダ別ルーティング）
│   │       │       │   ├── GmailProvider.ts
│   │       │       │   ├── OutlookProvider.ts
│   │       │       │   └── ResendProvider.ts
...
| Method | Path | 説明 |
|--------|------|------|
| GET | `/conversations` | 一覧（status, channel, assignee, mentioned, favorite, search でフィルタ） |
...
| POST | `/webhooks/outlook` | Microsoft Graph Notification |
...
export const Actions = {
  ...
  "webhook:receiveGmail":        "Gmail Push Notification の処理",
  "webhook:receiveOutlook":      "Outlook Notification の処理",
...
| bulk_email_drafts | id, tenant_id, subject, body, recipient_group_ids(JSONB), recipient_contact_ids(JSONB), created_by, state(draft/sent/deleted), recorded_at, superseded_at | uni | ソフト |
...
| line_message_usage | id, tenant_id, month(YYYY-MM), count, recorded_at, superseded_at | uni | ソフト |
...
- **双方向リンク**: `conversation_links` は A < B の順序ペアで複合PK。同一 `contact_id` の会話のみリンク可能（Service 層でバリデーション）
...
| プラン | 説明 | 制限例 |
| --- | --- | --- |
| Free | 無料 (¥0/月) | メンバー 1 名、チャネル 1 個、LINE 利用不可 |
| Starter | スターター (¥980/月) | チャネル 5 個まで、LINE 無料枠 200通/月（超過 3円/通） |
| Pro | プロ (¥2,980/月) | チャネル無制限、LINE 無料枠 1,000通/月（超過 3円/通） |
...
export type LimitableResource = "members" | "channels" | "monthlyMessages" | "monthlyLineMessages"
```
...
```typescript
// MessageService 内で LINE 送信時にカウント
const sendLineMessage = (params) =>
  pipe(
    ChannelRouter.resolve("line").send(params),
    Effect.tap(() => BillingService.recordLineUsage(params.tenantId, 1)),
  )
```

`BillingService.recordLineUsage()` は Stripe の `Usage Record` API を呼び出してメーター値をインクリメントする。
...
| Gmail API | 5xx, 429, タイムアウト | `standardRetry` | 429 は `Retry-After` ヘッダーを尊重 |
| Microsoft Graph API | 5xx, 429, タイムアウト | `standardRetry` | 429 は `Retry-After` ヘッダーを尊重 |
...
| Gmail API | fake Layer（EmailGateway） | OAuth トークンが必要なため fake で代替 |
| Microsoft Graph API | fake Layer（EmailGateway） | OAuth トークンが必要なため fake で代替 |
...
├── EmailGateway.ts            # Email 統合（プロバイダ別ルーティング）
│   ├── GmailProvider.ts       # Gmail API 実装
│   ├── OutlookProvider.ts     # Microsoft Graph API 実装
│   └── ResendProvider.ts      # Resend API 実装
├── LineGateway.ts             # LINE Messaging API 実装
├── InstagramGateway.ts        # Instagram Graph API 実装
...
POST /webhooks/:channel
  ...
  ├─ ConversationRepo.updateLastMessageAt + StatusMachine（new/in_progress → 変化なし, completed/no_action → new に再浮上）
...
- **チャネル接続方式**: 3種類の接続方法をサポート
  - Gmail: Google OAuth で認証、Gmail API で送受信
  - Outlook: Microsoft OAuth で認証、Microsoft Graph API で送受信
  - その他: SMTP/IMAP 手動設定（Resend をフォールバックとして利用可能）
- **送信**: 接続方式に応じて Gmail API / Microsoft Graph API / Resend API で送信
- **受信**: Gmail/Outlook は Push Notification (Webhook) or ポーリング、その他は Resend Inbound Webhook
...
- **添付ファイル**: メールプロバイダから取得したデータを R2 にアップロード
- **アカウント設定（`accounts.config` JSONB）**:
  ```json
  // Gmail
  { "provider": "gmail", "googleRefreshToken": "encrypted:...", "googleEmail": "user@gmail.com" }
  // Outlook
  { "provider": "outlook", "msRefreshToken": "encrypted:...", "msEmail": "user@outlook.com" }
  // その他（Resend）
  { "provider": "resend", "resendApiKey": "encrypted:...", "fromEmail": "support@example.com" }
  ```
...
CREATE INDEX idx_conversations_tenant_account_contact
  ON conversations (tenant_id, account_id, contact_id, last_message_at DESC);
